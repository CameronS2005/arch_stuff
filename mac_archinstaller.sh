...
DESKTOP_ENVIRONMENT="plasma" # (plasma,xfce, others...)
...
# Global variables
rel_date="UPDATE TIME; May 09, 5:00 PM EDT (2025)"
...
mac_base_packages="base base-devel networkmanager "$CPU_TYPE"-ucode sudo nano grub linux-t2 linux-t2-headers apple-t2-audio-config apple-bcm-firmware linux-firmware iwd grub efibootmgr t2fanrd"
...
pacstrap_install() {
    ...
    if [[ $isMAC == "true" ]]; then
        base_packages="$mac_base_packages"
    else
    ...
    pacstrap -i /mnt $base_packages $desktop_packages $custom_packages $gpu_drivers --noconfirm ## do we need to enable multilib for the installer environment?
}
